...
# エポック数と最小ロスの設定
# プレトレーニング（ランダムDVFで位置をずらした画像を生成して学習）
epochs = 1
best_loss = float('inf')
shift_range = 1
...
for epoch in tqdm(range(epochs)):
    ...
    # 8×16×16 の DVF を 128x256x256 にリサイズして、位置をずらした画像を生成するための DVF とする
    ...
    # 128x256x256 の DVF を 64x128x128 にリサイズして、モデルの出力と同じサイズにする
    ...
    moving_warped = transformer(moving_w, Vec)
    ...
    if epoch == 0 or (epoch + 1) % 10 == 0:
        progress_text = (
            ...
            f"Vec={loss_vec.detach().item():.6f}, "
            f"Image={loss_image.detach().item():.6f}, "
            f"Shift=±{shift_range}"
        )
        print(progress_text)
...
torch.save(
    model3D.state_dict(),
    'model_wavelet_pretrain_final.pth'
)
...
model3D.load_state_dict(
    torch.load('model_wavelet_pretrain_final.pth', map_location=device)
)
model3D.train()
optimizer = optim.Adam(model3D.parameters(), lr=1e-4)
...
for epoch in tqdm(range(epochs)):
    ...
    if (epoch + 1) % 1000 == 0:
        torch.save(
            ...
            'model_wavelet_finetune_checkpoint.pth'
        )
...
torch.save(
    model3D.state_dict(),
    'model_wavelet_finetune_final.pth'
)
